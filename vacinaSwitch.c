...
void buscaCPF(struct vacina * cadastro, int * nCadastro){
  ...
  scanf("%s", &cpfBuscado[30]);

  for (count = 0; count < * nCadastro;){
    if(strcmp(&cpfBuscado[30], cadastro[count].CPF) == 0){
      printf("\nCadastro encontrado!!! Dados da busca: \n");
      printf("\nNº Carteira Vacina: (%d)\nNome: %s\nCPF: %s\nFabricante: %s\nData: %s\nLote: %d \n",  cadastro[count].codeNumber, cadastro[count].name, cadastro[count].CPF, cadastro[count].model, cadastro[count].date, cadastro[count].lote);
      break;
    }else
...
int main(void) {
  ...
  switch(option){
    ...
    break;
    case 2: system("clear");
            listarVacinados(&cadastro, &nCadastro);
    break;
    case 3: system("clear");
            buscaCPF(&cadastro, &nCadastro);
    break;
